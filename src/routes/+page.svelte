<script>
	...
	import { MapLibre, FullScreenControl, GeoJSONSource, LineLayer, FillLayer } from "svelte-maplibre-gl";
	...
	async function searchLocation(searchString) {
		...
		// if result is unique, zoom to it
		if (searchSuggestions.length == 1) {
			search = searchSuggestions[0].name;
			zoomToLevel(9.5)(searchSuggestions[0].lat, searchSuggestions[0].lon);
	...
	async function handleSearchSubmit(event) {
		...
		debounce(await searchLocation(search), 1000);
	}
...
<div class="container flex flex-col gap-4 mx-auto my-4">
	...
	<form class="grid grid-cols-1 justify-stretch gap-4 flex-wrap align-stretch">
		<div class="flex gap-2 flex-wrap switcher" style="--threshold:30rem;">
			<button type="submit" class="text-white bg-black hover:bg-neutral-800 focus:ring-4 focus:outline-none focus:ring-neutral-300 font-medium rounded-lg text-sm px-4 py-2 self-center" onclick={prevDay}>Předchozí den</button>
			<div class="relative flex-1 !grow-4">
				<div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none min-w-6/12">
					<svg class="w-4 h-4 text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
						<path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z" />
					</svg>
				</div>
				<input type="date" id="date" class="block w-full p-4 ps-10 text-sm text-gray-900 rounded-lg border border-black" placeholder="YYYY-MM-DD" datepicker-format aria-label="Načíst data" required bind:value={date} max={yesterdayISO} />
				<button type="submit" class="text-white absolute end-2.5 bottom-2.5 bg-black hover:bg-neutral-800 focus:ring-4 focus:outline-none focus:ring-neutral-300 font-medium rounded-lg text-sm px-4 py-2" onclick={loadData}>Načíst Data</button>
			</div>
			<button
				type="submit"
				class="text-white bg-black hover:bg-neutral-800 focus:ring-4 focus:outline-none focus:ring-neutral-300 font-medium rounded-lg text-sm px-4 py-2 self-center
	...
	<MapLibre class="h-[75vh] min-h-[300px]" {style} {...initialState} bind:map bind:zoom bind:center>
		...
		<form class="search-form" onsubmit={handleSearchSubmit}>
			<div>
				<input type="search" id="location-search" name="q" placeholder="Zadejte adresu, město, obec nebo PSČ" list="searchSuggestions" bind:value={search} />
				<button type="submit" class="text-white bg-neutral-700 hover:bg-neutral-800 focus:ring-4 focus:outline-none focus:ring-neutral-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-neutral-600 dark:hover:bg-neutral-700 dark:focus:ring-neutral-800">Hledat</button>
				{#if searchSuggestions.length > 0}
					<div id="searchSuggestions">
						{#each searchSuggestions as suggestion, index (index)}
							<button class="suggestion" value={suggestion.name} onclick={() => handleSuggestionClick(suggestion)}>
								{suggestion.name}
							</button>
						{/each}
					</div>
		...
		<div id="state-legend" class="legend">
			...
			<div>
				<span style="background-color: {COLOR_SCALE[2]}"></span> Střední riziko
			</div>
			<div>
	...
	<section class="grid grid-cols-1 gap-4">
		...
		<p>
			Mapa zobrazuje riziko infekce zimostrázu (Buxus) patogenem <i>Cylindrocladium buxicola</i> a jeho šíření na základě vyhodnocení naměřených meteorologických dat (teploty vzduchu a četnosti srážek) a hodnot optimálních pro vývoj patogenu.
		</p>
		...
		<table class="w-full border max-w-fit">
			...
			<tbody>
				<tr class="border even:bg-gray-200 odd:bg-white">
					...
					<td class="px-4 py-2 flex items-center"><span class="w-3 h-3 rounded-full bg-[#ffffb2] mr-2"></span>žlutá</td>
				</tr>
				<tr class="border even:bg-gray-200 odd:bg-white">
					<td class="px-4 py-2">15–19,9 °C</td>
					<td class="px-4 py-2">střední</td>
					<td class="px-4 py-2 flex items-center"><span class="w-3 h-3 rounded-full bg-[#fd8d3c] mr-2"></span>oranžová</td>
				</tr>
				<tr class="border even:bg-gray-200 odd:bg-white">
					<td class="px-4 py-2">20–24 °C</td>
					<td class="px-4 py-2">vysoké</td>
					<td class="px-4 py-2 flex items-center"><span class="w-3 h-3 rounded-full bg-[#e31a1c] mr-2"></span>červená</td>
				</tr>
				<tr class="border even:bg-gray-200 odd:bg-white">
		...
		<p>
			K zobrazení dat z jednotlivých meteostanic je využit kartogram (choropleth).<br />Jednotkou zeměpisné oblasti je obec.
		</p>
